...
class AnimatedPTR extends React.Component {
  ...
  static propTypes = {
     ...
     */
     onScroll: PropTypes.func,
      /**
     * ScrollViiew, FlatList, SectionList
     * @type {string}
     */
     typeOfContentComponent: PropTypes.string
  }
  ...
  scrollTo(offset) {
    if (this.props.typeOfContentComponent === 'FlatList') {
      this.refs.PTR_ScrollComponent.scrollToOffset({ offset });
    } else if (this.props.typeOfContentComponent === 'SectionList') {
      this.refs.PTR_ScrollComponent.scrollToLocation({ viewOffset: offset, sectionIndex: 0, itemIndex: 0 });
    } else {
      this.refs.PTR_ScrollComponent.scrollTo({ y: offset });
    }
  }

  onScrollRelease() {
    if(!this.props.isRefreshing && this.state.shouldTriggerRefresh) {
      this.scrollTo(-this.props.minPullDistance);
    ...
    return (
      <View style={{flex:1, zIndex:-100,backgroundColor: this.props.contentBackgroundColor}}>
        ...
        { React.cloneElement(this.props.children, {
            show: this.props.isRefreshing || this.state.shouldTriggerRefresh,
            scrollY: this.state.scrollY,
            minPullDistance: this.props.minPullDistance
          })
        }
